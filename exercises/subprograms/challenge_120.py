...
If the user entered 2 as their selection on the menu, it should run a subprogram
...
def process(min1, max1, min2, max2, operation):
    ...
    (is_correct, correct_answer) = check_answer(num_1, num_2, operation, user_answer)
    display_response(is_correct, correct_answer)
    return is_correct
...
def challenge_execution():
    while True:
        user_input = get_user_input_integer(
            "Press 1 for addition, 2 for subtraction, 3 to terminate the program: "
        )
